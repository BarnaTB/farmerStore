...
export class OrderController{
    static async create(req:Request, res:Response, next:NextFunction){
            ...
            const {fertilizer_id, seed_id, fertilizer_quantity,seeds_quantity, fertilizer_price, seeds_price} = req.body
                const user = await UserService.getUser({id}) as Users
                const fertilizer = await FertilizerService.getUser({id:fertilizer_id}) as Fertilizers
                const seed = await SeedsService.get({id:seed_id}) as Seeds
                const fertilizerValidation = validateQuantity(fertilizer_quantity,fertilizer.quantity_limit,user.land_size, "fertilizer")
                const seedsValidation = validateQuantity(seeds_quantity, seed.quantity_limit, user.land_size, "seed")
                if(fertilizerValidation || seedsValidation){
                    return res.status(httpStatus.BAD_REQUEST).json({message: fertilizerValidation || seedsValidation })
                }
               const totalPriceOfOrder = (fertilizer_price * fertilizer_quantity) + (seeds_price * seeds_quantity)
               const details = {
                user_id:id,
                fertilizer_id,
                seed_id,
    ...
    static async getUserOrders(req:Request, res:Response, next:NextFunction){
        ...
        const {id} = req.user as Users
            const result = await OrderService.getOrdersCondition({user_id:id},page,itemsPerPage)
    ...
    static async changeOrderStatus(req:Request, res:Response, next:NextFunction){
