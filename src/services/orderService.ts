...
export class OrderService{
   ...
   static async getAll(query:{}, page:number, itemsPerPage:number){
     return await paginate(prisma.orders, query, {page,itemsPerPage},["users", "fertilizers", "seeds"])
   }
   static async getOrdersCondition(query:Partial<Orders>, page:number,itemsPerPage:number){
    return await paginate(prisma.orders,query,{page,itemsPerPage},["users", "fertilizers", "seeds"])
   }
